...
export const createBook = async (
  ...
  try {
    ...
    if (!validatedData.success) {
      throw new AppError(
        validatedData.error.issues[0]?.message || "Invalid book data",
        400
      );
    }
...
export const updateBook = async (
  ...
  try {
    ...
    if (activeRental) {
      throw new AppError(
        "Book cannot be updated while it has an active rental process",
        400
      );
    }
...
export const deleteBook = async (
  ...
  try {
    ...
    if (rentalCount > 0) {
      throw new AppError(
        "Book cannot be deleted because rental records exist",
        400
      );
    }
