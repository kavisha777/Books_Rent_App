...
export const createConditionRecord = async (
  ...
  try {
    ...
    const validatedData =
      createConditionSchema.safeParse(req.body);
    ...
    if (!allowedStatuses.includes(rental.status)) {
      throw new AppError(
        "A condition record cannot be created at this stage",
        400
      );
    }
    ...
    res.status(201).json({
      success: true,
      message:
        "Condition record created successfully",
      data: {
        conditionRecord,
      },
    });
...
export const getConditionRecords = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!isRenter && !isOwner && !isAdmin) {
      throw new AppError(
        "You do not have access to these condition records",
        403
      );
    }

    const conditionRecords =
      await prisma.conditionRecord.findMany({
        where: {
          rentalId,
        },
        orderBy: {
    ...
    res.status(200).json({
      success: true,
      message:
        "Condition records retrieved successfully",
      data: {
        conditionRecords,
