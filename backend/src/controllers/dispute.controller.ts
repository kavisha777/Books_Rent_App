...
export const createDispute = async (
  ...
  try {
    ...
    const validatedData =
      createDisputeSchema.safeParse(req.body);
    ...
    if (!allowedStatuses.includes(rental.status)) {
      throw new AppError(
        "A dispute cannot be created at this stage",
        400
      );
    }

    const existingOpenDispute =
    ...
    if (existingOpenDispute) {
      throw new AppError(
        "You already have an active dispute for this rental",
        400
      );
    }
...
export const getRentalDisputes = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!isRenter && !isOwner && !isAdmin) {
      throw new AppError(
        "You do not have access to these disputes",
        403
      );
    }
