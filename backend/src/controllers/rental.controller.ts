...
export const confirmPayment = async (
  ...
  try {
    ...
    if (rental.status !== RentalStatus.APPROVED) {
      throw new AppError(
        "Only approved rentals can proceed to payment",
        400
      );
    }
...
export const markHandover = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (rental.status !== RentalStatus.CONFIRMED) {
      throw new AppError(
        "Only confirmed rentals can proceed to handover",
        400
      );
    }
...
export const startRental = async (
  ...
  try {
    ...
    if (rental.status !== RentalStatus.HANDOVER_PENDING) {
      throw new AppError(
        "Rental must be in handover status first",
        400
      );
    }
...
export const requestReturn = async (
  ...
  try {
    ...
    if (rental.renterId !== req.user.userId) {
      throw new AppError(
        "Only the renter can request a return",
        403
      );
    }
...
export const startInspection = async (
  ...
  try {
    ...
    if (rental.ownerId !== req.user.userId) {
      throw new AppError(
        "Only the book owner can inspect the returned book",
        403
      );
    }
    ...
    if (rental.status !== RentalStatus.RETURN_PENDING) {
      throw new AppError(
        "Book must be returned before inspection",
        400
      );
    }
...
export const completeRental = async (
  ...
  try {
    ...
    if (rental.ownerId !== req.user.userId) {
      throw new AppError(
        "Only the book owner can complete the inspection",
        403
      );
    }
    ...
    if (rental.status !== RentalStatus.INSPECTION) {
      throw new AppError(
        "Rental must be under inspection before completion",
        400
      );
    }
