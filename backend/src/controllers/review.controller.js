...
export const createReview = async (req, res, next) => {
    try {
        ...
        const validatedData = createReviewSchema.safeParse(req.body);
        if (!validatedData.success) {
            throw new AppError(validatedData.error.issues[0]?.message ||
                "Invalid review data", 400);
        }
        ...
        if (existingReview) {
            throw new AppError("You have already reviewed this rental", 400);
        }
...
export const getRentalReviews = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError("Authentication required", 401);
        }
        ...
        if (!isRenter && !isOwner && !isAdmin) {
            throw new AppError("You do not have access to these reviews", 403);
        }
